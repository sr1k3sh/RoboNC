package com.arrtsm.app.woodapp.rendering;
...
public class ObjectRender1 {
    ...
    public ObjectRender1(String objPath) {
        OBJ_PATH = objPath;
    }
    ...
    public void createOnGlThread(Context context,String diffuseTextureAssetName
        ...
        // Read the obj file.
        String path = Environment.getExternalStorageDirectory().getAbsolutePath()+"/CNC DATA/model.obj";
        File file = new File(path);
       // File file1 = new File(path+"box1.mtl");
        Log.d("rikesh1", "createOnGlThread: "+file);
        FileInputStream fileInputStream = new FileInputStream(file);
        Log.d("rikesh1", "createOnGlThread: "+fileInputStream);
       // FileInputStream fileInputStream1 = new FileInputStream(file1);
        InputStream objInputStream = context.getAssets().open(OBJ_PATH);
        mObj = ObjReader.read(fileInputStream);
        Log.d("rikesh1", "createOnGlThread: "+mObj);
        if (mObj.getNumMaterialGroups() == 0 && mObj.getMtlFileNames().size() == 0) {
            Log.e("rikesh1", "No mtl file defined for this model.");
            return;
        }
        ...
        for (int i = 0; i < mObj.getNumMaterialGroups(); i++) {
            ...
            if (!mObj.getMtlFileNames().isEmpty()) {
                List<Mtl> mtlList = MtlReader.read(context.getAssets().open(parentDirectory + mObj.getMtlFileNames().get(0)));
                Log.d("rikesh", "createOnGlThread: "+mtlList);
                //context.getAssets().open(parentDirectory + mObj.getMtlFileNames().get(0)));
                Mtl targetMat = null;
                for (Mtl mat : mtlList) {
                    if (currentMatGroup.getName().equals(mat.getName())) {
                        targetMat = mat;
                        Log.d("rikesh", "createOnGlThread: ok"+targetMat.getMapKd());
                        break;
                    }
                ...
                if (targetMat.getMapKd() != null && !targetMat.getMapKd().isEmpty()) {
                    // Read the texture.
                    Log.d("rikesh", "createOnGlThread: test"+targetMat.getMapKd());
                    Bitmap textureBitmap;
                   // Log.d("ball",targetMat.getMapKd());
                    if (targetMat.getMapKd().contains("tga")) {
                        textureBitmap = readTgaToBitmap(context, parentDirectory + targetMat.getMapKd());
                    } else {
                        /*if(targetMat.getMapKd().equals("a.jpg")){
                            textureBitmap = BitmapFactory.decodeStream(context.getAssets().open(diffuseTextureAssetName));
                        }
                        else if(targetMat.getMapKd().equals("images.jpg")){
                            textureBitmap = BitmapFactory.decodeStream(context.getAssets().open(diffuseTextureAssetName));
                            //textureBitmap = BitmapFactory.decodeStream(context.getAssets().open(parentDirectory + targetMat.getMapKd()));
                        }
                        else*/
                            //textureBitmap = BitmapFactory.decodeStream(context.getAssets().open(diffuseTextureAssetName));
                            textureBitmap = BitmapFactory.decodeStream(context.getAssets().open(parentDirectory + targetMat.getMapKd()));
